...
num_games = 0
num_active = 0
f = open("Data/data_bits_drawed.csv", "w")
...
for i in range(102110423):
    ...
    result = get_result_draw(game)

    rating = (int(game.headers["WhiteElo"]) + int(game.headers["BlackElo"]))//2

    len_bits = len(list(game.mainline_moves()))-2

    if (len_bits >= 14 and "Bullet" not in game.headers["Event"] and rating > 2400 and "Normal" ==  game.headers["Termination"]):
        num_active += 1
